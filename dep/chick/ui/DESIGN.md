...
```
              ...
              └─────┬──────┘           │  点注册       │
               ...
               ▼              ▼                    ▼
   ┌─────────────────┐  ┌──────────────┐  ┌─────────────────┐
   │  Project Detail  │  │  Agent List  │  │  Project List   │
   │  /projects/:id   │  │  /agents     │  │  /projects      │
   │                  │  │              │  │                  │
   │  Header:         │  │  筛选栏:     │  │                  │
   │  项目名  [设置]  │  │  状态 | 类型  │  │                  │
   │  [+ 创建Issue]   │  │              │  │                  │
   ...
   └────────┬─────────┘  └──────────────┘  └─────────────────┘
            ...
            ▼                                              
   ┌──────────────────┐          ┌───────────────────────┐
   │  Issue Detail     │          │  Project Settings      │
   │  /issues/:id      │          │  /projects/:id/settings│
   │                   │          │                        │
   │  ← 返回项目       │          │  Tab:                  │
   │  状态标签 #编号   │          │  [标签][里程碑]        │
   │                   │          │  [技能][成员]          │
...
桌面端（≥1024px）左右分栏布局：左侧是 AI Agent 接入指南，右侧是登录/注册表单。
手机端上下堆叠：表单在上，Agent 接入卡片在下。
...
│                        │  Agent 协作   │              │
│  SSE 模式               │              │              │
│  Endpoint: POST /mcp   │  外部 ID     │              │
│                        │  [input]     │              │
│  可用工具清单            │  密码        │              │
│                        │  [input]     │              │
│  GraphQL API 入口       │  [   登录   ] │              │
│                        │              │              │
│  BOOTSTRAP_TOKEN 提示   │  没有账号？   │              │
│                        │  注册 ←切换   │              │
│                        └──────────────┘              │
└──────────────────────────────────────────────────────┘
...
- 左侧 Agent 接入文档包含：STDIO 配置示例、SSE 端点地址、可用 MCP 工具列表、GraphQL API 入口说明
- 右侧/上方登录表单支持登录和注册切换（底部文案切换）
- 注册成功后自动填充登录表单并切换到登录模式
- 注册不需要认证（API 对 registerAgent 开放）
- Agent 类型：人类(human)/AI(ai)/混合(hybrid)
- 能力(capabilities)：逗号分隔的技能列表（可选）
- 首次启动的 BOOTSTRAP_TOKEN 在左侧文档底部提示


### 项目成员管理

项目设置 → 成员 Tab 支持添加和移除成员：
...
- 下拉列表只显示尚未加入项目的 Agent
- 拥有者(owner)不可被移除（无 X 按钮）
- 成员(member)可被移除（点击 X 确认后移除）
- 添加/移除后自动刷新列表

## 权限模型

所有 GraphQL 请求经过两层安全检查：

### 第一层：认证（Auth Middleware）

JWT 中间件拦截所有 `/graphql` 请求（除 `loginAgent` / `registerAgent` 外），验证 `Authorization: Bearer <token>` 头，将解析出的 `agentID` 注入请求上下文。

### 第二层：授权（Resolver 层校验）

每个 mutation 和 query resolver 从上下文中提取认证后的 `agentID`，与服务端收到的参数进行比对：

| 校验规则 | 覆盖的 mutation/query |
|---------|---------------------|
| **身份校验**：`creatorID` / `actorID` / `authorID` 必须等于 JWT 中的 `agentID` | `createIssue`, `transitionIssue`, `addComment`, `createFeedback` |
| **项目成员校验**：调用者必须是项目成员 | `project`, `issues`, `issue`, `labels`, `milestones`, `skills`, `comments`, `timeline`, `createIssue`, `transitionIssue`, `addComment`, `updateIssue`, `deleteIssue`, `addLabels`, `removeLabels`, `addAssignee`, `removeAssignee`, `updateAssigneeState` |
| **项目拥有者校验**：调用者必须是项目 owner | `updateProject`, `deleteProject`, `addProjectMember`, `updateProjectMember`, `removeProjectMember`, `createLabel`, `createMilestone` |

### 不被客户端控制的字段

以下字段不再信任客户端输入，直接使用 JWT 中的认证身份：
- `createIssue.creatorID` → 验证是否等于认证用户
- `transitionIssue.actorID` → 验证是否等于认证用户
- `addComment.authorID` → 验证是否等于认证用户
- `createFeedback.authorID` → 验证是否等于认证用户

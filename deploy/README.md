# Dolphin Production Deployment

Deployment configs for Linux (systemd), macOS (launchd + Homebrew), and Windows (SC / nssm).
...
Or install from the provided formula:
...
### PowerShell background job (simple)
...
## All platforms — nohup (quick & dirty)
...
# Linux
sudo systemctl stop dolphin && sudo cp dolphin /usr/local/bin/dolphin && sudo systemctl start dolphin
# macOS launchd

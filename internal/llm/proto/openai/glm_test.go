...
func TestGLM_ChatCompletions(t *testing.T) {
	cfg, err := config.LoadConfig("../../../../config.yaml")
	if err != nil {
		t.Skipf("config not found: %v", err)
	}
	apiKey := cfg.GetString("llm.glm.api_key")
	if apiKey == "" {
		t.Skip("llm.glm not configured")
	}

	baseURL := cfg.GetString("llm.glm.base_url")
	model := cfg.GetString("llm.glm.models.0.name")
	if baseURL == "" || model == "" {
		t.Skip("llm.glm.base_url or models.0.name not configured")
	}

	t.Logf("glm: base_url=%s model=%s", baseURL, model)
	...
	req := llm.LLMRequest{
		...
		MaxTokens: 200, // GLM burns tokens on reasoning before output
		Stream:    false,
	}

	msgs := openaiproto.BuildMessages(req, nil)
	body, err := openaiproto.BuildRequest(model, msgs,
		llm.Config{Provider: "glm", BaseURL: baseURL, APIKey: apiKey}, req)
	if err != nil {

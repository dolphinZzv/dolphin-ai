...
// BuiltinMCPHandlers returns handlers for MCP tools that are built into
// the main process rather than running as external subprocesses.
func BuiltinMCPHandlers() map[string]BuiltinHandler {
...
func handleShell(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	var req struct {

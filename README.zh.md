...
dolphin-ai 不在乎你敲哪个门 —— 它都会应答。同一个 agent、同一套工具、同一份会话状态，无论走哪个传输层。
...
dolphin-ai 支持五种传输协议，可以同时启用：

- **stdio** — 默认。运行 `./dolphin-ai` 在终端聊天。首次运行会引导你设置职业画像和推荐工具。
- **SSH** — 远程连接。`ssh dolphin-ai@host -p 2222`。同样的 agent 会话，终端界面。
...
dolphin-ai 支持 **Linux**、**macOS** 和 **Windows**（arm64 和 x86_64）。完整安装方式（预编译二进制、go install 等）见 [INSTALL.zh.md](docs/zh/INSTALL.zh.md)。
...
# Windows（PowerShell）
go build -o dolphin-ai.exe .   # 开发版本

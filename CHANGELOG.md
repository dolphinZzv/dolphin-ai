...
- **AgentMesh 自注册空地址冲突修复**: `NewAgentMesh` 仅在 `ListenAddr` 非空时自注册，防止空地址与 mock 真实地址冲突被 tie-breaker 误判。`StaticHandler` 返回 A2A 格式 `{id, artifacts}` 修复 `Delegate` 返回空 content。

- **全局 LLM 身份标识 header**: `providerHeaders` 对所有 provider 自动注入 `HTTP-Referer: https://dolphin.siciv.space/` 和 `X-Title: Dolphin-AI`，不再限于 OpenRouter。内置 headers 先 apply，用户自定义 headers 后覆盖。
...
- **Memory WAL 时光机**: 新增 `WALMemory`（`tidwall/wal` append-only 存储 + 轻量 index），通过 `session.type: wal` 启用。WAL 为唯一数据源，index 只存 24 bytes/条 offset。Write→gob/WAL append，Replace→compact 快照保留完整历史。`WriteTurn` 记录 turn 元数据，`TurnMarks` 查看时间线，`RewindTo` 安全回溯（写新 compact）。GC 每日 1 次，`session.wal_retention`/`session.wal_keep_turns` 可配置。11 个单元测试全覆盖（CRUD/重启/并发/回溯/GC）。`tools/session-inspect` 提供内嵌 HTML 的 HTTP 查看器（`go run ./tools/session-inspect` → http://localhost:9090），支持 Timeline/Raw/Diff 三种视图，6 个单元测试覆盖。

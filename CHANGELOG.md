...
## [Unreleased]
- Remove dead code flagged by `unused`: `errPermissionDenied` sentinel, `limitStore` interface, `FileStore.save` method, `mcpServerConfig` struct, `_removed` test stub, unused `models`/`ctx`/`cancel`/`mu` fields

...
## [Unreleased]
- **`/brain push` / `/brain pull` / `/brain set url <url>`**: git operations on the brain repository. Brain is a git-versioned knowledge directory — `/brain push` pushes commits to origin, `/brain pull` fetches and merges from origin, `/brain set url <url>` configures the remote. Aliases `/push` and `/pull` at root level.
